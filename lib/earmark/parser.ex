defmodule Earmark.Parser do
  ...
  private do
    ...
    def footnote_def?(%Block.FnDef{}), do: true
    def footnote_def?(_block), do: false

    @spec find_footnote_links(Block.t) :: list(String.t)
    def find_footnote_links(%Block.Para{lines: lines, lnb: lnb}) do
      lines
      |> Enum.zip(Stream.iterate(lnb, &(&1 + 1)))
      |> Enum.flat_map(&extract_footnote_links/1)
    end
    def find_footnote_links(%{blocks: blocks}) do
      Enum.flat_map(blocks, &find_footnote_links/1)
    end
    def find_footnote_links(_), do: []

    @spec extract_footnote_links({String.t, number()}) :: list({String.t, number()})
    def extract_footnote_links({line, lnb}) do
      Regex.scan(~r{\[\^([^\]]+)\]}, line)
     |> Enum.map(&tl/1)
    ...
    def create_footnote_blocks(blocks, []), do: blocks

    def create_footnote_blocks(blocks, footnotes) do
      lnb = footnotes
            |> Stream.map(&(&1.lnb)) 
            |> Enum.min()
