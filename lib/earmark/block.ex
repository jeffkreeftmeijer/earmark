defmodule Earmark.Block do
  ...
  def assign_attributes_to_blocks([ block | rest], result) do
    ...
    def _parse([], result, options), do: {result, options}

    ###################
    # setext headings #
    ###################

    def _parse([  %Line.Blank{},
    ...
    def _parse([ %Line.Heading{content: content, level: level, lnb: lnb} | rest ], result, options) do
    ...
    def _parse([ %Line.Ruler{type: type, lnb: lnb} | rest], result, options) do
    ...
    def _parse( lines = [ %Line.BlockQuote{lnb: lnb} | _ ], result, options) do
    ...
    def _parse( lines = [ %Line.TableLine{columns: cols1, lnb: lnb1},
    ...
    def _parse( lines = [ %Line.TableLine{lnb: lnb} | _ ], result, options) do
    ...
    def _parse( lines = [ %Line.Text{lnb: lnb} | _ ], result, options)
    do
    ...
    def _parse( [first = %Line.ListItem{type: type, initial_indent: initial_indent, content: content, bullet: bullet, lnb: lnb} | rest ], result, options) do
    ...
    def _parse( list = [%Line.Indent{lnb: lnb} | _], result, options) do
    ...
    def _parse([%Line.Fence{delimiter: delimiter, language: language, lnb: lnb} | rest], result, options) do
    ...
    # HTML block #
    ##############
    def _parse([ opener = %Line.HtmlOpenTag{tag: tag, lnb: lnb} | rest], result, options) do
    ...
    def _parse([ %Line.HtmlOneLine{line: line, lnb: lnb} | rest], result, options) do
    ...
    def _parse([ line = %Line.HtmlComment{complete: true, lnb: lnb} | rest], result, options) do
      _parse(rest, [ %HtmlOther{html: [ line.line ], lnb: lnb} | result ], options)
    end

    def _parse(lines = [ %Line.HtmlComment{complete: false, lnb: lnb} | _], result, options) do
    ...
    def _parse( [ defn = %Line.IdDef{title: title, lnb: lnb}, maybe_title | rest ], result, options)
    when title == nil
    do
    ...
    # or not
    def _parse( [ defn = %Line.IdDef{lnb: lnb} | rest ], result, options) do
    ...
    def _parse( [ defn = %Line.FnDef{id: _id, lnb: lnb} | rest ], result , options) do
    ...
    def _parse( [ %Line.Ial{attrs: attrs, lnb: lnb, verbatim: verbatim} | rest ], result, options) do
    ...
    def _parse( [ %Line.Blank{} | rest ], result, options) do
    ...
    def _parse( lines = [%Line.Plugin{prefix: prefix, lnb: lnb}|_], result, options) do
    ...
    def _parse( [ anything = %{lnb: lnb} | rest ], result, options) do
    ...
    def consolidate_para( lines ), do: _consolidate_para( lines, [], @not_pending )

    @spec _consolidate_para( ts, ts, inline_code_continuation ) :: { ts, ts, inline_code_continuation }
    def _consolidate_para( [], result, pending ) do
      {result, [], pending}
    end

    def _consolidate_para( [line | rest] = lines, result, pending ) do
    ...
    def consolidate_list_items([], result) do
    ...
    def consolidate_list_items([ item = %ListItem{type: type} | rest], result) do
      start = extract_start(item)
    ...
    def compute_list_spacing( list = %List{blocks: items} ) do
    ...
    def compute_list_spacing( anything_else ), do: anything_else # nop

    def any_spaced_items?([]), do: false
    def any_spaced_items?([%{spaced: true}|_]), do: true
    def any_spaced_items?([_|tail]), do: any_spaced_items?(tail)
    ...
    def read_table([ %Line.TableLine{columns: cols} | rest ],
                      col_count,
                      table = %Table{})
    ...
    def read_table( rest, col_count, %Table{rows: rows}) do
    ...
    def look_for_alignments([ _first, second | _rest ]) do
    ...
    def links_from_blocks(blocks) do
      visit(blocks, Map.new, &link_extractor/2)
    end

    @spec link_extractor(t, %{}) :: %{}
    def link_extractor(item = %IdDef{id: id}, result) do
      Map.put(result, String.downcase(id), item)
    end

    def link_extractor(_, result), do: result
    ...
    def html_match_to_closing(opener, rest), do: find_closing_tags([opener], rest, [opener])

    # No more open tags, happy case
    def find_closing_tags([], rest, html_lines), do: {html_lines, rest, []}

    # run out of input, unhappy case
    def find_closing_tags(needed, [], html_lines), do: {html_lines, [], needed}

    # still more lines, still needed closing
    def find_closing_tags(needed = [needed_hd|needed_tl], [rest_hd|rest_tl], html_lines) do
      cond do
    ...
    def collect_plugin_lines(lines, prefix, result)
    def collect_plugin_lines([], _, result), do: {Enum.reverse(result), []}
    def collect_plugin_lines([%Line.Plugin{prefix: prefix, content: content, lnb: lnb} | rest], prefix, result),
      do: collect_plugin_lines(rest, prefix, [{content, lnb} | result])
    def collect_plugin_lines( lines, _, result ), do: {Enum.reverse(result), lines}

    ###########
    # Helpers #
    ###########

    def closes_tag?(%Line.HtmlCloseTag{tag: ctag}, %Line.HtmlOpenTag{tag: otag}), do: ctag == otag
    def closes_tag?(_, _), do: false

    def opens_tag?(%Line.HtmlOpenTag{}), do: true
    def opens_tag?(_), do: false


    # (_,{'nil' | binary(),number()}) -> #{}jj
    @spec inline_or_text?( Line.t, inline_code_continuation ) :: %{pending: String.t, continue: boolean}
    def inline_or_text?(line, pending)
    def inline_or_text?(line = %Line.Text{}, @not_pending) do
      pending = opens_inline_code(line)
      %{pending: pending, continue: true}
    end
    def inline_or_text?(line = %Line.TableLine{}, @not_pending) do
      pending = opens_inline_code(line)
      %{pending: pending, continue: true}
    end
    def inline_or_text?( _line, @not_pending), do: %{pending: @not_pending, continue: false}
    def inline_or_text?( line, pending ) do
      pending = still_inline_code(line, pending)
      %{pending: pending, continue: true}
    end


    def peek([], _, _), do: false
    def peek([head | _], struct, type) do
    ...
    def remove_trailing_blank_lines(lines) do
      lines
      |> Enum.reverse
